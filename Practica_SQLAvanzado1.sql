...
SELECT SAFE_CAST(ivr_calls.ivr_id AS STRING) AS calls_ivr_id
      ...
      ,SAFE_CAST(FORMAT_DATE('%Y%m%d', ivr_calls.start_date) AS STRING) calls_start_date_id
      ,ivr_calls.end_date AS calls_end_date
      ,SAFE_CAST(FORMAT_DATE('%Y%m%d', ivr_calls.end_date) AS STRING) calls_end_date_id
